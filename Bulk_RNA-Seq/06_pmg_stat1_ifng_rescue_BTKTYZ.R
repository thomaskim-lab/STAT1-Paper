############################################################
### Primary mouse microglia STAT1 siRNA + IFN RNA-seq analysis
### Plasmidsaurus order: BTKTYZ
### Species: Mouse / Mus musculus
###
### Samples:
###   PMG_Lipo_96h_R1/R2/R3
###   PMG_Lipo_96h_IFN_R1/R2/R3
###   PMG_siSTAT1_3_96h_R1/R2/R3
###   PMG_siSTAT1_3_96h_IFN_R1/R2/R3
###
### Main analysis uses all 12 samples.
### DE contrasts include IFN response, STAT1 siRNA effect,
### and the siSTAT1_3 x IFN interaction.
############################################################
...
outdir <- file.path(
  base_dir,
  "BTKTYZ_PrimaryMicroglia_STAT1_siRNA_IFN_96h_analysis"
)
...
if (file.exists(sample_map_file)) {
  ...
  sample_map <- data.frame(
    ...
    Sample_name = c(
      ...
      "PMG_Lipo_96h_IFN_R2",
      "PMG_Lipo_96h_IFN_R3",
      "PMG_siSTAT1_3_96h_R1",
      "PMG_siSTAT1_3_96h_R2",
      "PMG_siSTAT1_3_96h_R3",
      "PMG_siSTAT1_3_96h_IFN_R1",
      "PMG_siSTAT1_3_96h_IFN_R2",
      "PMG_siSTAT1_3_96h_IFN_R3"
    ),
    stringsAsFactors = FALSE
  )
  write.csv(
    sample_map,
    file = file.path(outdir, "Generated_BTKTYZ_sample_map.csv"),
    row.names = FALSE
  )
}
...
if (all(matrix_ids %in% sample_map$run_id)) {
  idx <- match(matrix_ids, sample_map$run_id)
  ...
  idx <- match(matrix_ids, sample_map$Sample_name)
  sample_map <- sample_map[idx, , drop = FALSE]
  colnames(count_mat) <- sample_map$Sample_name
} else {
  missing_run_ids <- setdiff(matrix_ids, sample_map$run_id)
  missing_sample_names <- setdiff(matrix_ids, sample_map$Sample_name)
  stop(
    "Expression-matrix columns do not match sample_map run_id or Sample_name.\n",
    "Missing as run_id: ", paste(missing_run_ids, collapse = ", "), "\n",
    "Missing as Sample_name: ", paste(missing_sample_names, collapse = ", ")
  )
}

if (!is.null(cpm_mat)) {
  cpm_ids <- colnames(cpm_mat)

  if (all(sample_map$run_id %in% cpm_ids)) {
    cpm_idx <- match(sample_map$run_id, cpm_ids)
    cpm_mat <- cpm_mat[, cpm_idx, drop = FALSE]
    colnames(cpm_mat) <- sample_map$Sample_name
  } else if (all(sample_map$Sample_name %in% cpm_ids)) {
    cpm_idx <- match(sample_map$Sample_name, cpm_ids)
    cpm_mat <- cpm_mat[, cpm_idx, drop = FALSE]
    colnames(cpm_mat) <- sample_map$Sample_name
  }
}
...
parse_sample_metadata <- function(sample_names) {
  tibble(Sample_name = sample_names) %>%
    mutate(
      Condition = case_when(
        grepl("^PMG_Lipo_96h_R[0-9]+$", Sample_name) ~ "Lipo_96h",
        grepl("^PMG_Lipo_96h_IFN_R[0-9]+$", Sample_name) ~ "Lipo_96h_IFN",
        grepl("^PMG_siSTAT1_3_96h_R[0-9]+$", Sample_name) ~ "siSTAT1_3_96h",
        grepl("^PMG_siSTAT1_3_96h_IFN_R[0-9]+$", Sample_name) ~ "siSTAT1_3_96h_IFN",
        TRUE ~ "Other"
      ),
      TransfectionContext = case_when(
        Condition %in% c("Lipo_96h", "Lipo_96h_IFN") ~ "LipoOnly",
        Condition %in% c("siSTAT1_3_96h", "siSTAT1_3_96h_IFN") ~ "RNAiMAX_siRNA",
        TRUE ~ "Other"
      ),
      siRNA = case_when(
        Condition %in% c("Lipo_96h", "Lipo_96h_IFN") ~ "Control",
        Condition %in% c("siSTAT1_3_96h", "siSTAT1_3_96h_IFN") ~ "siSTAT1_3",
        TRUE ~ "Other"
      ),
      IFN = case_when(
        Condition %in% c("Lipo_96h_IFN", "siSTAT1_3_96h_IFN") ~ "IFN",
        Condition %in% c("Lipo_96h", "siSTAT1_3_96h") ~ "No_IFN",
        TRUE ~ "Other"
      ),
      Duplex = case_when(
        Condition %in% c("Lipo_96h", "Lipo_96h_IFN") ~ "None",
        Condition %in% c("siSTAT1_3_96h", "siSTAT1_3_96h_IFN") ~ "DsiRNA_3",
        TRUE ~ "Other"
      ),
    ...
    mutate(
      Condition = factor(
        Condition,
        levels = c("Lipo_96h", "Lipo_96h_IFN", "siSTAT1_3_96h", "siSTAT1_3_96h_IFN")
      ),
      TransfectionContext = factor(
        TransfectionContext,
        levels = c("LipoOnly", "RNAiMAX_siRNA")
      ),
      siRNA = factor(siRNA, levels = c("Control", "siSTAT1_3")),
      IFN = factor(IFN, levels = c("No_IFN", "IFN")),
      Duplex = factor(Duplex, levels = c("None", "DsiRNA_3")),
...
print(table(meta_all$siRNA, meta_all$IFN))
...
p_pca_all <- ggplot(
  ...
  ggtitle("PCA - Primary mouse microglia STAT1 siRNA + IFN, 96h") +
  theme_bw()

ggsave(
  file.path(outdir, "PCA_all_samples.png"),
  p_pca_all,
  width = 7,
  height = 5,
  dpi = 300
...
annotation_all <- as.data.frame(colData(dds_all)[, c("Condition", "siRNA", "IFN"), drop = FALSE])
...
############################################################
### 10. Main analysis: all 12 samples
############################################################

main_samples <- meta_all %>%
  filter(Condition %in% c("Lipo_96h", "Lipo_96h_IFN", "siSTAT1_3_96h", "siSTAT1_3_96h_IFN")) %>%
  pull(Sample_name)
...
meta_main$IFN <- droplevels(meta_main$IFN)
...
write.xlsx(
  meta_main,
  file = file.path(outdir, "Parsed_sample_metadata_main_analysis.xlsx"),
  overwrite = TRUE
)

############################################################
### 10A. Condition-level DESeq2 model
############################################################
...
############################################################
### 10B. Factorial DESeq2 model for siSTAT1_3 x IFN interaction
############################################################

dds_factorial <- DESeqDataSetFromMatrix(
  countData = count_main,
  colData = meta_main,
  design = ~ siRNA + IFN + siRNA:IFN
)

dds_factorial <- dds_factorial[keep, ]
dds_factorial <- DESeq(dds_factorial)
print(resultsNames(dds_factorial))

rn_factorial <- resultsNames(dds_factorial)
interaction_name <- rn_factorial[
  grepl("siRNA", rn_factorial) &
    grepl("IFN", rn_factorial) &
    !grepl("_vs_", rn_factorial)
]

if (length(interaction_name) != 1) {
  stop(
    "Could not uniquely identify the siRNA:IFN interaction term. resultsNames(dds_factorial): ",
    paste(rn_factorial, collapse = ", ")
  )
}

message("Using factorial interaction coefficient: ", interaction_name)
...
res_IFN_vs_noIFN_in_Lipo <- results(
  dds,
  contrast = c("Condition", "Lipo_96h_IFN", "Lipo_96h")
)

res_IFN_vs_noIFN_in_siSTAT1_3 <- results(
  dds,
  contrast = c("Condition", "siSTAT1_3_96h_IFN", "siSTAT1_3_96h")
)

res_siSTAT1_3_vs_Lipo_96h <- results(
  dds,
  contrast = c("Condition", "siSTAT1_3_96h", "Lipo_96h")
)

res_siSTAT1_3_vs_Lipo_96h_IFN <- results(
  dds,
  contrast = c("Condition", "siSTAT1_3_96h_IFN", "Lipo_96h_IFN")
)

res_interaction_siSTAT1_3_x_IFN <- results(
  dds_factorial,
  name = interaction_name
)

res_IFN_vs_noIFN_in_Lipo_df <- make_res_table(res_IFN_vs_noIFN_in_Lipo, gene_annot)
res_IFN_vs_noIFN_in_siSTAT1_3_df <- make_res_table(res_IFN_vs_noIFN_in_siSTAT1_3, gene_annot)
res_siSTAT1_3_vs_Lipo_96h_df <- make_res_table(res_siSTAT1_3_vs_Lipo_96h, gene_annot)
res_siSTAT1_3_vs_Lipo_96h_IFN_df <- make_res_table(res_siSTAT1_3_vs_Lipo_96h_IFN, gene_annot)
res_interaction_siSTAT1_3_x_IFN_df <- make_res_table(res_interaction_siSTAT1_3_x_IFN, gene_annot)

contrast_results <- list(
  IFN_vs_noIFN_in_Lipo = res_IFN_vs_noIFN_in_Lipo_df,
  IFN_vs_noIFN_in_siSTAT1_3 = res_IFN_vs_noIFN_in_siSTAT1_3_df,
  siSTAT1_3_vs_Lipo_96h = res_siSTAT1_3_vs_Lipo_96h_df,
  siSTAT1_3_vs_Lipo_96h_IFN = res_siSTAT1_3_vs_Lipo_96h_IFN_df,
  Interaction_siSTAT1_3_x_IFN = res_interaction_siSTAT1_3_x_IFN_df
)

write_xlsx_safe(
  contrast_results,
  file = file.path(outdir, "DE_results_STAT1_siRNA_IFN_96h.xlsx")
)

############################################################
### 12. Normalized counts and VST
############################################################
...
############################################################
### 13. PCA and sample distance for main analysis
############################################################
...
ggsave(
  file.path(outdir, "PCA_main_analysis.png"),
  p_pca,
  width = 7,
  height = 5,
  dpi = 300
...
pcaData_factorial <- plotPCA(vsd, intgroup = c("siRNA", "IFN"), returnData = TRUE)
percentVar_factorial <- round(100 * attr(pcaData_factorial, "percentVar"))

p_pca_factorial <- ggplot(
  pcaData_factorial,
  aes(PC1, PC2, color = siRNA, shape = IFN, label = name)
) +
  geom_point(size = 4) +
  ggrepel::geom_text_repel(size = 3) +
  xlab(paste0("PC1: ", percentVar_factorial[1], "% variance")) +
  ylab(paste0("PC2: ", percentVar_factorial[2], "% variance")) +
  ggtitle("PCA - Primary mouse microglia siRNA / IFN factors, 96h") +
  theme_bw()

ggsave(
  file.path(outdir, "PCA_main_analysis_siRNA_IFN_factors.png"),
  p_pca_factorial,
  width = 7,
  height = 5,
  dpi = 300
...
annotation_df <- as.data.frame(colData(dds)[, c("Condition", "siRNA", "IFN"), drop = FALSE])
...
pheatmap(
  ...
  main = "Sample-to-sample distances: main analysis"
)
dev.off()
...
volcano_titles <- c(
  IFN_vs_noIFN_in_Lipo = "Primary microglia 96h: IFN vs no IFN in Lipo",
  IFN_vs_noIFN_in_siSTAT1_3 = "Primary microglia 96h: IFN vs no IFN in siSTAT1 #3",
  siSTAT1_3_vs_Lipo_96h = "Primary microglia 96h: siSTAT1 #3 vs Lipo",
  siSTAT1_3_vs_Lipo_96h_IFN = "Primary microglia 96h + IFN: siSTAT1 #3 vs Lipo",
  Interaction_siSTAT1_3_x_IFN = "Primary microglia 96h: siSTAT1 #3 x IFN interaction"
)
...
############################################################
### 16. Focused primary microglia / STAT1 / lipid modules
############################################################
...
p_module_mean <- ggplot(module_summary_all, aes(x = Module, y = mean_log2FC, fill = Contrast)) +
  ...
  labs(
    title = "Primary microglia STAT1 siRNA + IFN 96h: focused module mean log2FC",
    x = "",
    y = "Mean log2FC"
  )
...
if (nrow(module_annot) > 0) {
  ...
  pheatmap(
    ...
    cluster_cols = FALSE,
    main = "Focused microglia / STAT1 / IFN / lipid / redox modules"
  )
  dev.off()
}

############################################################
### 19. Marker CPM summary
############################################################
...
if (!is.null(cpm_mat)) {
  ...
  marker_cpm_long <- as.data.frame(cpm_mat[keep_marker_genes, , drop = FALSE]) %>%
    ...
    left_join(meta_all %>% dplyr::select(Sample_name, Condition, siRNA, IFN, Replicate), by = "Sample_name")
...
make_safe_sheet_names <- function(x) {
  # Replace Excel-invalid characters
  x <- gsub("[\\[\\]\\*\\?/\\\\:]", "_", x)
...
message("Primary mouse microglia STAT1 siRNA + IFN 96h analysis completed successfully.")
message("Output directory: ", outdir)
message("All samples processed: ", ncol(count_mat))
message("Main-analysis samples: ", ncol(count_main))
message("Genes retained in main DESeq2 model: ", nrow(dds))
message("Factorial interaction coefficient used: ", interaction_name)
